...
{
  Structure_Details *sd = calloc(numPages,sizeof(Structure_Details)); //allocating memory
  ...
  do{
    sd[i].data = NULL;
    sd[i].pagenum = np;
    sd[i].dirtyPage = initialize;
    sd[i].clientcount = initialize;
    sd[i].freq_used = initialize;
    sd[i].nr = initialize;
    i=i+1;
